...
6. `python3 .claude/hooks/workflow.py verify`를 실행한다. 필요한 성능 결과는 조건·측정값·변경 전후 비교를 포함해 검사 로그에 출력한다. 상세 자료는 작업의 evidence/에 보관한다. 필수 성능 테스트를 실행할 수 없으면 block으로 전환하고 미완료로 보고한다.
7. 실패하면 `evidence/check-*.log`를 읽고 수정한 뒤 다시 검사한다. 통과하면 `review-task`로 이어간다.
