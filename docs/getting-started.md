...
이 저장소의 앱은 아직 만들지 않았습니다. 실제 프로젝트에서는 harness.json의 활성 프로필과 경로를 프로젝트 구조에 맞춥니다.
...
`harness.json`에서 기술 프로필을 활성화하고 프로젝트 전용 검사를 등록합니다.
...
자세한 스키마와 기본 프로필은 [범용 설정](configuration.md)을 참고하세요. `harness.json`이 없는 기존 설치만 `.claude/checks.json`을 사용합니다. 실제 앱의 테스트 도구가 지원하는 명령으로 조정해야 하며 빈 검사 목록은 통과할 수 없습니다.
...
harness.json에는 **일반 테스트 다음에 성능 테스트**를 별도 검사로 등록합니다. verify는 첫 필수 실패·시간 초과에 멈추고 남은 명령을 미실행으로 기록합니다. 성능 기준 미달 시 성능 명령은 0이 아닌 종료 코드를 반환해야 합니다.
...
- Agent 매처와 `subagent_type` 필드는 001 작업에서 실제 세션으로 확인했습니다(docs/experiments/001-harness-hardening.md). NotebookEdit 매처와 `notebook_path`는 아직 미확인입니다.
...
- `.env`와 `.env.local`은 기본 추가 입력입니다. 외부 서비스·환경 변수·DB 상태 등 파일 밖 입력은 지문으로 증명하지 못합니다.
...
- `.github/workflows/harness.yml`은 하네스 자체의 회귀 테스트를 CI에서 실행합니다. 실제 앱을 이식하면 앱 검사·보호 브랜치의 필수 체크·배포 게이트를 별도 설정해야 합니다.
