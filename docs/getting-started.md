...
- 목표·범위·완료 기준이 비어 있거나, 기준 표에 데이터 행이 없거나, plan-review.md 판정·계획 단계 verifier 호출 기록이 없으면 최초 구현 시작 거부(wait·block을 거쳐도 동일)
...
- evidence/, state.json, active.json, tasks/index.md는 모든 단계에서 도구 수정 거부 (settings.json permissions.deny와 Hook 이중)
...
- 필수 검사 명령을 실제로 실행하고 종료 코드·로그·시각·소요 시간·Python 버전·git HEAD를 evidence/checks.json에, 변경 파일 목록을 evidence/changes.txt에 저장
